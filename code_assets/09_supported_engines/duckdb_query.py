"""
Feature 9: DuckDB — query Snowflake Horizon Catalog via Iceberg REST extension
DuckDB >= 1.1.0 ships with the iceberg extension that speaks Iceberg REST.

pip install duckdb

Set env vars:
  SNOWFLAKE_TOKEN = <OAuth token>
"""

import os
import duckdb

HORIZON_URI = "https://scb47336.snowflakecomputing.com/polaris/api/catalog"
WAREHOUSE   = "horizon_demo_db"
TOKEN       = os.getenv("SNOWFLAKE_TOKEN")

conn = duckdb.connect()
conn.execute("INSTALL iceberg; LOAD iceberg;")
conn.execute(f"""
    CREATE OR REPLACE SECRET horizon_secret (
        TYPE      ICEBERG_REST,
        TOKEN     '{TOKEN}',
        ENDPOINT  '{HORIZON_URI}',
        WAREHOUSE '{WAREHOUSE}'
    );
""")
conn.execute(f"""
    ATTACH '{HORIZON_URI}' AS horizon (
        TYPE   ICEBERG_REST,
        SECRET 'horizon_secret'
    );
""")

print(conn.execute("SHOW ALL TABLES").fetchdf().to_string(index=False))
print(conn.execute("SELECT * FROM horizon.public.transactions").fetchdf().to_string(index=False))
conn.close()
